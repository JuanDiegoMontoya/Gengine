...
void main()
{
  ...
  if (any(greaterThanEqual(gid, u_targetDim)))
    return;
  vec3 uvw = (vec3(gid) + 0.5) / u_targetDim;

  uvw.z = InvertDepthZO(uvw.z, 1.0, 100.0);
  vec3 p = UnprojectUV(uvw.z, uvw.xy, u_invViewProj);
  vec3 d = vec3((snoise(p / 10.0) + 1.0) / 10.0);
  //vec3 d = vec3(0);
  ...
  d *= 1. - smoothstep(30, 80, p.y);
  imageStore(i_target, gid, vec4(d, 1));
}
